...
def check_integration_files():
    ...
    success, output = run_docker_command("docker exec homeassistant ls -la /config/custom_components/easyiq/")
    if success:
    ...
    # Check manifest.json
    success, output = run_docker_command("docker exec homeassistant cat /config/custom_components/easyiq/manifest.json")
    if success:
        try:
